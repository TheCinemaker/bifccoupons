...
/* ======================== Banggood adapter ======================== */
function signBanggood(params: Record<string, any>): string { const sorted = Object.keys(params).sort().map(k => `${k}=${params[k]}`).join("&"); return crypto.createHash("md5").update(sorted).digest("hex"); }
async function bgGetAccessToken(): Promise<string | null> {
    if (!BANGGOOD_API_KEY || !BANGGOOD_API_SECRET) return null;
    const timestamp = Math.floor(Date.now() / 1000); const noncestr = Math.random().toString(36).slice(2);
...
async function fetchBanggoodDeals(): Promise<Deal[]> {
  ...
  while (page <= pages) {
    ...
    for (const c of list) {
      ...
      const deal: Deal = {
        id: `banggood:${crypto.createHash("md5").update(`${stdRaw}|${c.coupon_code || ""}`).digest("hex")}`, src: "banggood", title: name, image: c.coupon_img || undefined, url: finalUrl, short: shortHttps || undefined,
...
// A régi, működő kódod alapján a helyes munkalap nevekkel!
const SHEET_RANGES = [
  "'Banggood unique'!A1:Z500", 
  "'unique_2'!A1:Z500",
  "'geekbuying'!A1:Z500",
  "'geekbuying Unique'!A1:Z500",
  "'ALIEXPRESS1111'!A1:Z500"
];
function findIdx(header: string[], aliases: string[]): number { const lower = header.map(h => String(h).trim().toLowerCase()); for (const a of aliases) { const i = lower.indexOf(a.toLowerCase()); if (i !== -1) return i; } return -1; }
async function fetchSheetsDeals(): Promise<Deal[]> {
  ...
  try {
    ...
    const jwt = new google.auth.JWT(creds.client_email, undefined, privateKey, ["https://www.googleapis.com/auth/spreadsheets.readonly"]);
    ...
    for (const vr of resp.data.valueRanges || []) {
      const rows = vr.values || []; if (rows.length < 2) continue;
      const header = rows[0].map((h: any) => String(h));
      const iImage = findIdx(header, ["Image"]); const iName = findIdx(header, ["Product name", "Name"]); const iLink = findIdx(header, ["Link", "URL"]); const iPrice = findIdx(header, ["Coupon price", "Price"]); const iCode = findIdx(header, ["Coupon code", "Code"]); const iWh = findIdx(header, ["Warehouse"]); const iEnd = findIdx(header, ["End time", "End", "Expiry", "Expire"]); const iOrig = findIdx(header, ["Original price"]);
      
      for (let r = 1; r < rows.length; r++) {
        const row = rows[r];
        const title = iName !== -1 ? row[iName] : undefined; const link = iLink !== -1 ? row[iLink] : undefined;
        if (!title || !link) continue;
        
        const endIso = iEnd !== -1 ? toISO(row[iEnd]) : undefined; if (endIso && new Date(endIso).getTime() <= Date.now()) continue;
        
        const imageRaw = iImage !== -1 ? row[iImage] : undefined; const imageUrl = extractImageUrl(imageRaw);
        const price = iPrice !== -1 ? parseMoney(row[iPrice]) : undefined; const code = iCode !== -1 ? row[iCode] : undefined; const wh = iWh !== -1 ? row[iWh] : undefined; const orig = iOrig !== -1 ? parseMoney(row[iOrig]) : undefined;
        
        const safeLink = normalizeUrl(String(link)) || String(link);
        out.push({
          id: `sheets:${crypto.createHash("md5").update(`${vr.range}|${safeLink}|${code || ""}`).digest("hex")}`, src: "sheets", title: String(title), image: imageUrl, orig, url: safeLink, price, cur: "USD", code: code || undefined, wh: wh || undefined, end: endIso
        });
      }
    }
    const items = dedupe(out); SHEETS_CACHE = { items, ts: Date.now() }; return items;
  } catch (error: any) {
    console.error("!!! KRITIKUS HIBA a fetchSheetsDeals közben:", error.message);
    return []; // Hiba esetén üres listát adunk vissza, hogy az oldal ne haljon el.
  }
}
function dedupe(deals: Deal[]): Deal[] {
    const seen = new Map<string, Deal>();
    // A sorrend fontos: a sheetsDeals-t adjuk hozzá először!
    for (const d of deals) {
        const key = `${normalizeUrl(d.url)}|${d.code || ""}`;
        if (!seen.has(key) || d.src === 'sheets') {
            seen.set(key, d);
        }
    }
...
export const handler: Handler = async (event) => {
  try {
    ...
    const minPrice = qs.get("minPrice") ? Number(qs.get("minPrice")) : undefined; const maxPrice = qs.get("maxPrice") ? Number(qs.get("maxPrice")) : undefined;
    
    const sheetsDeals = await fetchSheetsDeals();
    const banggoodDeals = await fetchBanggoodDeals();
    
    let all = dedupe([...sheetsDeals, ...banggoodDeals]);

    if (all.length === 0) { all = [{ id: "demo-1", src: "sheets", title: "BlitzWolf BW-XYZ 65W GaN töltő", url: "https://example.com", price: 39.99, }]; }
    ...
    const start = cursor ? parseInt(cursor, 10) || 0 : 0; const page = scored.slice(start, start + limit);
    ...
    return { statusCode: 200, headers: { "Content-Type": "application/json", "Cache-Control": "public, max-age=600, stale-while-revalidate=60", "ETag": etg }, body: json };
  } catch (e: any) {
