...
const SHEET_RANGES = ["'BG Unique'!A1:Z500", "'BG Unique HUN'!A1:Z500"];
...
async function fetchSheetsDeals(): Promise<Deal[]> {
  ...
  for (const vr of resp.data.valueRanges || []) {
    ...
    const iName = findIdx(header, ["Product name", "Name"]); const iLink = findIdx(header, ["Link", "URL"]); const iPrice = findIdx(header, ["Coupon price", "Price"]); const iCode = findIdx(header, ["Coupon code", "Code"]); const iWh = findIdx(header, ["Warehouse"]); const iEnd = findIdx(header, ["End time", "End", "Expiry", "Expire"]);
    for (let r = 1; r < rows.length; r++) {
      ...
      const imageRaw = row[0]; const imageUrl = extractImageUrl(imageRaw);
      const price = iPrice !== -1 ? parseMoney(row[iPrice]) : undefined; const code = iCode !== -1 ? row[iCode] : undefined; const wh = iWh !== -1 ? row[iWh] : undefined;
      const safeLink = normalizeUrl(String(link)) || String(link);
      out.push({
        id: `sheets:${crypto.createHash("md5").update(`${vr.range}|${safeLink}|${code || ""}`).digest("hex")}`, src: "sheets", title: String(title), image: imageUrl, url: safeLink, price, cur: "USD", code: code || undefined, wh: wh || undefined, end: endIso
      });
    }
  }
...
function dedupe(deals: Deal[]): Deal[] {
  ...
  for (const d of deals) { const key = `${d.url}|${d.code || ""}`; if (!seen.has(key)) { seen.add(key); out.push(d); } }
  return out;
}
/* ======================== Handler ======================== */
...
export const handler: Handler = async (event) => {
  try {
    ...
    const [sheetsDeals, banggoodDeals] = await Promise.all([ fetchSheetsDeals().catch(() => []), fetchBanggoodDeals().catch(() => []) ]);
